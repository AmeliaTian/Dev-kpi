...
@RestController
@Api(tags = "BASE_'AU11_用户角色映射表'", description = "'AU11_用户角色映射表'增删改查的api")
public class Au11UserRoleCtl {

    @Resource(name = "au11UserRoleService")
    private IAu11UserRoleService au11UserRoleService;

    @Resource(name = "shiroService")
    private AuthShiroService shiroService;

    @ApiOperation(value = "查询'AU11_用户角色映射表'记录数", notes = "根据传入参数查询'AU11_用户角色映射表'记录数,输入参数参照类Au11UserRoleVO")
    @RequestMapping(value = "/auth/au11UserRole", method = RequestMethod.HEAD)
    public HttpOutMsgBean count(HttpServletRequest request) {
        ...
        Long dataLen = au11UserRoleService.countEntity(paramMap);
    ...
    @ApiOperation(value = "查询'AU11_用户角色映射表'", notes = "##### 根据传入参数查询'AU11_用户角色映射表'，输入参数参照类Au11UserRoleVO的属性名，可通过添加后缀来实现‘等于’、‘大于’、‘LIKE’等操作。具体如下：\n" +
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Au11UserRoleVO.*", value = "Au11UserRoleVO实体中相关属性名", dataType = "*", paramType = "query"),
    ...
    @RequestMapping(value = "/auth/au11UserRole", method = RequestMethod.GET)
    public HttpOutMsgBean query(HttpServletRequest request, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sort", required = false) String sort) {
        ...
        Iterable<Au11UserRoleVO> queryResult = au11UserRoleService.queryEntity(paramMap, pageable);
    ...
    @ApiOperation(value = "新增'AU11_用户角色映射表'", notes = "批量新增,输入参数为Au11UserRoleVO的列表")
    @RequestMapping(value = "/auth/au11UserRole", method = RequestMethod.POST)
    public HttpOutMsgBean save(@RequestBody List<Au11UserRoleVO> au11UserRoleVOS) {
        Integer saveRows = au11UserRoleService.saveEntity(au11UserRoleVOS);
    ...
    @ApiOperation(value = "更新'AU11_用户角色映射表'", notes = "批量更新,输入参数为Au11UserRoleVO的列表")
    @RequestMapping(value = "/auth/au11UserRole", method = RequestMethod.PUT)
    public HttpOutMsgBean update(@RequestBody List<Au11UserRoleVO> au11UserRoleVOS) {
        //如果有ID为空，则不更新
        for (Au11UserRoleVO vo : au11UserRoleVOS) {
        ...
        Integer saveRows = au11UserRoleService.updateEntity(au11UserRoleVOS);
        shiroService.refreshUserRoles();
    ...
    @ApiOperation(value = "删除'AU11_用户角色映射表'", notes = "批量删除，传入参数为对应记录ID列表")
    @RequestMapping(value = "/auth/au11UserRole", method = RequestMethod.DELETE)
    public HttpOutMsgBean remove(@RequestBody List<String> ids) {
        Integer delRows = au11UserRoleService.removeEntity(ids);
        shiroService.refreshUserRoles();
