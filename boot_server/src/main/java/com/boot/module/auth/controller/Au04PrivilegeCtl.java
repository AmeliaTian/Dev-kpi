...
/**
 * @author CodeGen
 * @Description AU04_权限(资源表)表 Controller
 * @CreateDate 创建时间： 2018-09-18 14:44:51
 * @ModifiedBy
 * @ModifiedDate
 */
@RestController
@Api(tags = "BASE_'AU04_权限(资源表)表'", description = "'AU04_权限(资源表)表'增删改查api")
public class Au04PrivilegeCtl {
    @Resource(name = "au04PrivilegeService")
    private IAu04PrivilegeService au04PrivilegeService;

    @ApiOperation(value = "查询'AU04_权限(资源表)表'记录数", notes = "根据传入参数查询'AU04_权限(资源表)表'记录数,输入参数参照类Au04PrivilegeVO")
    @RequestMapping(value = "auth/au04Privilege", method = RequestMethod.HEAD)
    public HttpOutMsgBean count(HttpServletRequest request) {
        ...
        Long dataLen = au04PrivilegeService.countEntity(paramMap);
    ...
    @ApiOperation(value = "查询'AU04_权限(资源表)表'", notes = "##### 根据传入参数查询'AU04_权限(资源表)'，输入参数参照类Au04PrivilegeVO的属性名，可通过添加后缀来实现‘等于’、‘大于’、‘LIKE’等操作。具体如下：\n" +
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Au04PrivilegeVO.*", value = "Au04PrivilegeVO实体中相关属性名", dataType = "*", paramType = "query"),
    ...
    @RequestMapping(value = "auth/au04Privilege", method = RequestMethod.GET)
    public HttpOutMsgBean query(HttpServletRequest request, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sort", required = false) String sort) {
        ...
        Iterable<Au04PrivilegeVO> queryResult = au04PrivilegeService.queryEntity(paramMap, pageable);
    ...
    @ApiOperation(value = "新增'AU04_权限(资源表)表'", notes = "批量新增,输入参数为Au04PrivilegeVO的列表")
    @RequestMapping(value = "auth/au04Privilege", method = RequestMethod.POST)
    public HttpOutMsgBean save(@RequestBody List<Au04PrivilegeVO> au04PrivilegeVOS) {
        Integer saveRows = au04PrivilegeService.saveEntity(au04PrivilegeVOS);
    ...
    @ApiOperation(value = "更新'AU04_权限(资源表)表'", notes = "批量更新,输入参数为Au04PrivilegeVO的列表")
    @RequestMapping(value = "auth/au04Privilege", method = RequestMethod.PUT)
    public HttpOutMsgBean update(@RequestBody List<Au04PrivilegeVO> au04PrivilegeVOS) {
        //如果有ID为空，则不更新
        for (Au04PrivilegeVO vo : au04PrivilegeVOS) {
        ...
        Integer saveRows = au04PrivilegeService.updateEntity(au04PrivilegeVOS);
    ...
    @ApiOperation(value = "删除'AU04_权限(资源表)表'", notes = "批量删除，传入参数为对应记录ID列表")
    @RequestMapping(value = "auth/au04Privilege", method = RequestMethod.DELETE)
    public HttpOutMsgBean remove(@RequestBody List<String> ids) {
        Integer delRows = au04PrivilegeService.removeEntity(ids);
