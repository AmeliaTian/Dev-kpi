...
/**
 * @author CodeGen
 * @Description AU13_机构角色映射表 Controller
 * @CreateDate 创建时间： 2018-09-13 16:00:38
 * @ModifiedBy
 * @ModifiedDate
 */
@RestController
@Api(tags = "BASE_'AU13_机构角色映射表'", description = "'AU13_机构角色映射表'增删改查api")
public class Au13OrgRoleCtl {

    @Resource(name = "au13OrgRoleService")
    private IAu13OrgRoleService au13OrgRoleService;

    @Resource(name = "shiroService")
    private AuthShiroService shiroService;
    @ApiOperation(value = "查询'AU13_机构角色映射表'记录数", notes = "根据传入参数查询'AU13_机构角色映射表'记录数,输入参数参照类Au13OrgRoleVO")
    @RequestMapping(value = "/auth/au13OrgRole", method = RequestMethod.HEAD)
    public HttpOutMsgBean count(HttpServletRequest request) {
        ...
        Long dataLen = au13OrgRoleService.countEntity(paramMap);
    ...
    @ApiOperation(value = "查询'AU13_机构角色映射表'", notes = "##### 根据传入参数查询'AU13_机构角色映射表'，输入参数参照类Au13OrgRoleVO的属性名，可通过添加后缀来实现‘等于’、‘大于’、‘LIKE’等操作。具体如下：\n" +
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Au13OrgRoleVO.*", value = "Au13OrgRoleVO实体中相关属性名", dataType = "*", paramType = "query"),
    ...
    @RequestMapping(value = "/auth/au13OrgRole", method = RequestMethod.GET)
    public HttpOutMsgBean query(HttpServletRequest request, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sort", required = false) String sort) {
        ...
        Iterable<Au13OrgRoleVO> queryResult = au13OrgRoleService.queryEntity(paramMap, pageable);
    ...
    @ApiOperation(value = "新增'AU13_机构角色映射表'", notes = "批量新增,输入参数为Au13OrgRoleVO的列表")
    @RequestMapping(value = "/auth/au13OrgRole", method = RequestMethod.POST)
    public HttpOutMsgBean save(@RequestBody List<Au13OrgRoleVO> au13OrgRoleVOS) {
        Integer saveRows = au13OrgRoleService.saveEntity(au13OrgRoleVOS);
    ...
    @ApiOperation(value = "更新'AU13_机构角色映射表'", notes = "批量更新,输入参数为Au13OrgRoleVO的列表")
    @RequestMapping(value = "/auth/au13OrgRole", method = RequestMethod.PUT)
    public HttpOutMsgBean update(@RequestBody List<Au13OrgRoleVO> au13OrgRoleVOS) {
        //如果有ID为空，则不更新
        for (Au13OrgRoleVO vo : au13OrgRoleVOS) {
        ...
        Integer saveRows = au13OrgRoleService.updateEntity(au13OrgRoleVOS);
        shiroService.refreshUserRoles();
    ...
    @ApiOperation(value = "删除'AU13_机构角色映射表'", notes = "批量删除，传入参数为对应记录ID列表")
    @RequestMapping(value = "/auth/au13OrgRole", method = RequestMethod.DELETE)
    public HttpOutMsgBean remove(@RequestBody List<String> ids) {
        Integer delRows = au13OrgRoleService.removeEntity(ids);
        shiroService.refreshUserRoles();
