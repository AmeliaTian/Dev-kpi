 ...
 */
@RestController
@Api(tags="BASE_'部门管理表'",description = "'部门管理表'增删改查api")
public class Jx21BmglInfoCtl {

    @Resource(name = "jx21BmglInfoService")
    private IJx21BmglInfoService jx21BmglInfoService;

    @ApiOperation(value = "查询'部门管理表'记录数", notes = "根据传入参数查询'部门管理表'记录数,输入参数参照类Jx21BmglInfoVO")
    @RequestMapping(value = "/kpi/jx21BmglInfo", method = RequestMethod.HEAD)
    public HttpOutMsgBean count(HttpServletRequest request) {
        ...
        Long dataLen = jx21BmglInfoService.countEntity(paramMap);
    ...
    @ApiOperation(value = "查询'部门管理表'", notes = "##### 根据传入参数查询'部门管理表'，参数名称参照Jx21BmglInfoVO中的属性名，可通过添加后缀来实现‘等于’、‘大于’、‘LIKE’等操作。具体如下：\n" +
    ...
    @ApiImplicitParams({
        @ApiImplicitParam(name = "Jx21BmglInfoVO.*", value = "Jx21BmglInfoVO实体中相关属性名", dataType = "*", paramType = "query"),
    ...
    @RequestMapping(value = "/kpi/jx21BmglInfo", method = RequestMethod.GET)
    public HttpOutMsgBean query(HttpServletRequest request,@RequestParam(value = "page",required = false) Integer page, @RequestParam(value = "size",required = false) Integer size, @RequestParam(value = "sort",required = false) String sort) {
        ...
        Iterable<Jx21BmglInfoVO> queryResult =  jx21BmglInfoService.queryEntity(paramMap, pageable);
...
/*如果可编辑(table)，则生成下边的更新操作*/
    @ApiOperation(value = "新增'部门管理表'", notes = "批量新增,输入参数为Jx21BmglInfoVO的列表")
    @RequestMapping(value = "/kpi/jx21BmglInfo", method = RequestMethod.POST)
    public HttpOutMsgBean save(@RequestBody List<Jx21BmglInfoVO> jx21BmglInfoVOS) {
        Integer saveRows = jx21BmglInfoService.saveEntity(jx21BmglInfoVOS);
    ...
    @ApiOperation(value = "更新'部门管理表'", notes = "批量更新,输入参数为Jx21BmglInfoVO的列表")
    @RequestMapping(value = "/kpi/jx21BmglInfo", method = RequestMethod.PUT)
    public HttpOutMsgBean update(@RequestBody List<Jx21BmglInfoVO> jx21BmglInfoVOS) {
        //如果有ID为空，则不更新
        for (Jx21BmglInfoVO vo : jx21BmglInfoVOS) {
        ...
        Integer saveRows =  jx21BmglInfoService.updateEntity(jx21BmglInfoVOS);
    ...
    @ApiOperation(value = "删除'部门管理表'", notes = "批量删除，传入参数为对应记录ID列表")
    @RequestMapping(value = "/kpi/jx21BmglInfo", method = RequestMethod.DELETE)
    public HttpOutMsgBean remove(@RequestBody List<String> ids) {
        Integer delRows =  jx21BmglInfoService.removeEntity(ids);
