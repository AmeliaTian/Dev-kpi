 ...
 * @author h3dwy
 * @Description
 * @CreateDate 创建时间：2018-08-13 18:15
 * @ModifiedBy
 * @ModifiedDate
 */
...
public class TreeUtils {
    ...
    private static <T> void find(List<T> list, T parent, Field keyField, Field parentKeyField, Field subField) {
        ...
        if (subs != null) {
            ...
            //递归找子节点
            subs.forEach(sub -> find(list, sub, keyField, parentKeyField, subField));
        }
    }
    ...
    private static <T, E> List<E> findKeys(List<T> list, List<E> keys, T parent, Field keyField, Field parentKeyField) {
        ...
        if (subs != null) {
            keys.addAll(subKeys);
            //递归找子节点
            subs.forEach(sub -> findKeys(list, keys, sub, keyField, parentKeyField));
        }
        return keys;
    }
